...
public class CircuitBoard {
	...
	public CircuitBoard(String filename) throws FileNotFoundException, InvalidFileFormatException {
		...
		try { // ensure that first two tokens in file can be successfully converted to integers
			ROWS = Integer.parseInt(fileScan.next());
			COLS = Integer.parseInt(fileScan.next());
		} catch(Exception e) { // if first two tokens are not integers
		...
		for(int i = 0; i<ROWS; i++) { // go through each row
			fileScan.nextLine();
			for(int j = 0; j<COLS; j++) { // go through each column
				if(!fileScan.hasNext()) { // make sure file has a next value
					throw new InvalidFileFormatException(filename);
				}
				board[i][j] = fileScan.next().charAt(0);
				if(board[i][j]==START) { //set starting point
					startingPoint = new Point(i,j);
					startCount++;
				} else if(board[i][j]==END) { //set ending point
					endingPoint = new Point(i,j);
					endCount++;
				} else if(board[i][j]!=OPEN&&board[i][j]!=CLOSED&&board[i][j]!=TRACE) {//invalid character
					throw new InvalidFileFormatException(filename);
				}
			}
		}
		if(startCount!=1||endCount!=1) { // Not the right number of starts or ends
			throw new InvalidFileFormatException(filename);
		}
		if(fileScan.hasNext()) { // Not the right number of starts or ends
			throw new InvalidFileFormatException(filename);
		}
